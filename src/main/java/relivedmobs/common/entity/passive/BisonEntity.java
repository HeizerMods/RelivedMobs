...
public class BisonEntity extends AnimalEntity implements IAnimatable{

	public BisonEntity(EntityType<? extends AnimalEntity> mob, World world) {
		super(mob, world);
	}
	...
	public static MutableAttribute createBisonAttributes() {
		MutableAttribute attributes = AnimalEntity.createLivingAttributes()
	                .add(Attributes.MAX_HEALTH, 15)
	                .add(Attributes.MOVEMENT_SPEED, 0.4F)
	                .add(Attributes.ATTACK_SPEED, 2.5D);
		attributes.combine(AnimalEntity.createMobAttributes());
		return attributes;
	}
	    ...
	    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
	        if (event.isMoving()) {
	            ...
	            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bison.idle", true));
	        }
